...
class App(Router):
    ...
    def __call__(self, environ, start_response):
        """WSGI entrypoint."""
        resp = self._get_response_handled(self.request_handler, Request(environ), self.response_class())
    ...
    def request_router(self, request, response):
    ...
    def error_handler(self, request, http_error):
        ...
        if route:
            ...
            pass
        elif self.tracebacks_to_http and hasattr(http_error, 'traceback'):
            return self.traceback_handler(request, http_error)
        else:
            return self.generic_error_handler(request, http_error)

    def nested_error(self, request, first_error, second_error):
        http_error.headers['Content-type'] = 'text/plain'
        http_error.write("An error occurred:\n%s\n\n" % (first_error))
        http_error.write("Then an error occurred in the error handler:\n%s\n\n" % (second_error))

    def traceback_handler(self, request, http_error):
        http_error.headers['Content-type'] = 'text/plain'
        http_error.write("An error occurred:\n\n %s" % (http_error.traceback))

    def generic_error_handler(self, request, http_error):
